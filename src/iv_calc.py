...
def get_placeholder():
    ...
    portfolios = pd.merge(
        portfolios,
        futures_raw[["name_fut", "maturity",'id']],
        left_on='id_fut',
        right_on='id',
        how='left',
        suffixes=("", "_fut"),
    ...
    futures_raw = futures_raw[futures_raw['id'].isin(list(portfolios['id_fut'].unique()))]
...
def calculate_iv(opt_prc, uly_price, strike, tte, r, opt_type,symbol):
    if opt_prc < 0:
        return None
    try:
